...
. Selecciona tu exchange preferido del menú desplegable
. Ingresa tus credenciales (nombre de usuario y contraseña)
...
. Navega a la página *Cuenta*
. Encuentra la sección *Exchanges*
. Para tu exchange seleccionado, ingresa:
+
...
. Haz clic en *Guardar* para almacenar tus credenciales
...
. Inicia sesión en tu https://www.binance.com[cuenta de Binance]
. Haz clic en el ícono de tu perfil en la esquina superior derecha
. Selecciona *Gestión de API* del menú desplegable
. Haz clic en el botón *Crear API*
. Elige la opción de clave API *Generada por el Sistema*
. Ingresa una etiqueta para tu clave API (por ejemplo, "Bot de Trading")
. Completa la verificación de seguridad (2FA, email, etc.)
. Una vez creada, *copia y guarda inmediatamente la Clave API y la Clave Secreta*
+
[WARNING]
====
¡La Clave Secreta solo se muestra una vez! Guárdala de forma segura antes de cerrar la ventana.
====
. Haz clic en *Editar restricciones* para configurar los permisos:
+
...
. Haz clic en *Guardar* para aplicar las restricciones

===== KuCoin

. Inicia sesión en tu https://www.kucoin.com[cuenta de KuCoin]
. Haz clic en el ícono de tu perfil en la esquina superior derecha
. Selecciona *Gestión de API* del menú desplegable
. Haz clic en el botón *Crear API*
...
. Inicia sesión en tu https://www.mexc.com[cuenta de MEXC]
. Haz clic en el ícono de tu perfil en la esquina superior derecha
. Selecciona *Gestión de API* o *API* del menú
...
. Completa la verificación de seguridad (2FA, email, etc.)
. Una vez creada, *copia y guarda inmediatamente la Clave de Acceso y la Clave Secreta*
+
[WARNING]
====
¡La Clave Secreta solo se muestra una vez! Guárdala de forma segura antes de cerrar la ventana.
...
. Haz clic en *Confirmar* para guardar la clave API
...
. Navega a *Bots* → *Nuevo Bot*
. Configura todos los parámetros requeridos (detallados a continuación)
. Haz clic en *Crear Bot* para inicializar
. El bot se creará en estado pausado
...
*Posiciones Altas* (Requerido, default: 1)
* Número de posiciones de precio más alto analizadas para determinar la zona de precio alto
* Se utiliza para evitar recomprar cuando se vende a precios inflados
* Rango: 1 a (Posiciones Máximas - 1)
* Valores más altos = zona de precio alto más amplia, estrategia de recompra más conservadora
* *Recomendación*: Comienza con 1-2 para trading conservador
...
. Toma tus N mejores posiciones (basado en la configuración "Posiciones Altas")
. Identifica el rango de precio de estas posiciones más altas
. Calcula un límite inferior usando el porcentaje de umbral
. Si el precio de venta cae dentro de esta zona de precio alto, la recompra es *omitida*
...
. *Posiciones Libres Disponibles*: Existe al menos una posición libre
. *No Detenido*: El indicador "Detener Compras" no está establecido
. *Verificación de Rango de Precio*: El precio actual está dentro del rango de precio de trabajo min/max (si está establecido)
...
. *Precio Objetivo Alcanzado*: El precio objetivo de una compra ha sido alcanzado o superado
...
. Navega a la página *Trading*
. Selecciona un bot de tu lista
. Ve el historial de transacciones en la pestaña *Transacciones*
...
. Selecciona los bots que deseas controlar usando casillas
. Haz clic en *Pausar* para pausar solo esos bots
. Tus otros bots continúan operando normalmente
. Haz clic en *Detener Compra en Caída* para prevenir compras por caída de precio solo en bots seleccionados
...
*Acceder a Registros:*
. Navega a *Admin* → *Registros* (requiere permisos de administrador)
. Selecciona usuario y bot
. Ve el flujo de registro en tiempo real
...
*Ver Saldos:*
. Navega a la página *Saldos*
. Ve holdings actuales en exchanges
. Ve saldos disponibles y en uso
...
. Ve a la página *Cuenta*
. Haz clic en *Gestionar Configuraciones de Bot*
. Haz clic en *Descargar JSON*
. Las configuraciones se guardan en un archivo JSON con marca de tiempo

*Importar Configuraciones:*
. Ve a la página *Cuenta*
. Haz clic en *Gestionar Configuraciones de Bot*
. Haz clic en *Cargar JSON*
. Selecciona tu archivo JSON previamente exportado
. El sistema valida y crea bots a partir de las configuraciones
...
*Soluciones:*
. *Verificar Claves API*: Asegúrate de que las claves API estén correctamente configuradas
. *Verificar Permisos*: Las claves API deben tener permisos de trading
. *Verificar Saldo*: Asegúrate de que haya saldo suficiente en USDT para al menos una posición
. *Revisar Par de Trading*: Verifica que el par de trading esté disponible en el exchange
. *Revisar Registros*: Revisa los registros del bot para mensajes de error específicos
...
*Causas Posibles:*
. *Sin Posiciones Libres*: Todas las posiciones están llenas
+
** Solución: Espera ventas o aumenta las posiciones máximas

. *Detener Compras Activo*: Verifica si el modo "Detener Compra en Caída" o "Detener Compra en Recompra" está habilitado
+
** Solución: Haz clic en "Reanudar Compra en Caída" y/o "Reanudar Compra en Recompra" según sea apropiado

. *Precio Fuera de Rango*: El precio actual está fuera del rango de trabajo
+
** Solución: Ajusta los precios de trabajo min/max

. *Saldo Insuficiente*: No hay suficiente USDT para una posición
+
** Solución: Añade fondos o reduce el tamaño de posición

. *Precio No Ha Bajado Lo Suficiente*: Esperando el umbral de caída de precio
+
...
*Causas Posibles:*
. *Precio Objetivo No Alcanzado*: El precio no ha subido lo suficiente para ganancia
+
** Solución: Espera o reduce el margen de ganancia

. *Compras Pausadas*: Algunas compras pueden estar pausadas manualmente
+
** Solución: Reanuda compras pausadas

. *Tendencia Bajista del Mercado*: El precio está por debajo de precios de compra
+
...
*Causas Posibles:*
. *Saldo Insuficiente*: Fondos insuficientes para órdenes de compra
. *Cantidad Muy Pequeña*: Tamaño de orden por debajo del mínimo del exchange
+
** Solución: Aumenta el tamaño de posición o la inversión máxima

. *Cantidad Muy Grande*: La orden excede el saldo disponible
. *Límites de Tasa de API*: Demasiadas solicitudes al exchange
+
** Solución: Aumenta el intervalo del bot

. *Problemas del Exchange*: Problemas temporales del exchange
+
...
*Explicaciones:*
. *Comisiones*: Las comisiones de compra y venta reducen la ganancia
. *Deslizamiento*: Las órdenes de mercado pueden ejecutarse a precios ligeramente diferentes
. *Ganancias como Criptomoneda*: Parte de la ganancia se mantiene como cripto, reduciendo la ganancia en USDT
. *Redondeo*: Cantidades pequeñas pueden redondearse diferentemente
...
. Extractos de registros recientes
...
. *Comienza Pequeño*: Comienza con inversiones pequeñas para aprender el sistema
. *Usa Modo Simulación*: Prueba estrategias antes de usar fondos reales
. *Diversifica*: No pongas todos los fondos en un bot o par de trading
. *Establece Expectativas Realistas*: Los mercados cripto son volátiles
. *Monitorea Regularmente*: Verifica el rendimiento del bot diariamente
. *Mantén Reservas*: No asignes 100% de tu saldo del exchange

=== Consejos de Configuración

. *Configuración Conservadora*: Comienza con márgenes de ganancia más altos y menos posiciones
. *Rango de Trabajo*: Siempre establece precios de trabajo min/max para evitar compras en extremos
. *Número de Posiciones*: Más posiciones = mejor promedio pero llenado de posición más lento
. *Margen de Ganancia*: Cubre al menos 2x las comisiones de trading (ej: 0.2% comisión = 0.5% margen mínimo)
. *Intervalo del Bot*: 5-10 segundos es óptimo para la mayoría de escenarios

=== Seguridad

. *Claves API*: Nunca compartas tus claves API
. *Permisos de Retiro*: Desactiva los permisos de retiro en las claves API
. *Restricciones de IP*: Habilita lista blanca de IP en el exchange si está disponible
. *Auditorías Regulares*: Revisa regularmente las transacciones del bot
. *Contraseñas Fuertes*: Usa contraseñas fuertes y únicas para la plataforma

=== Mantenimiento

. *Revisiones Regulares*: Verifica el rendimiento del bot semanalmente
. *Actualizaciones de Configuración*: Ajusta parámetros según condiciones del mercado
. *Gestión de Saldo*: Asegúrate de saldo suficiente para operaciones del bot
. *Revisión de Registros*: Revisa registros para errores o advertencias
. *Actualizaciones de Software*: Mantén la plataforma actualizada (contacta al administrador)
